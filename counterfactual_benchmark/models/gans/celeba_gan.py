...
class Decoder(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.starting_constant = nn.Parameter(torch.ones((1, 512, 4, 4)))
        self.initial_adain1 = AdaIN(512, 512)
        self.initial_adain2 = AdaIN(256, 512) #the out channels of convt2, latent_dim
        self.initial_noise1 = InjectNoise(512) #the out channels of conv1
        self.initial_noise2 = InjectNoise(256) #the out channels of convt2
        
        self.initial_conv1 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1)
        self.initial_convt2 =  WSConvTranspose2d(512,256,7,2)
        self.initial_convt3 =  WSConvTranspose2d(256,256,5,2)
        self.initial_convt4 =  WSConvTranspose2d(256,128,7,2)
        self.initial_convt5 =  WSConvTranspose2d(128,256,2,1)
        self.leaky = nn.LeakyReLU(0.2, inplace=True)
        
        
        
        self.blocks=nn.ModuleList([])
        for _ in range(self.synt_blocks):
            self.blocks.append(GenBlock(in_channels=256,out_channels=256,w_dim=512).to('cuda'))
                               
        self.rgb_layer = WSConv2d(in_channels = 256, out_channels = 3, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, u, cond):
        ...
        x = self.initial_convt2(x)
        x = self.initial_convt3(x)
        x = self.initial_convt4(x)
        x = self.initial_convt5(x)
